...
create_vm () {
    ...
    qm create $vmid_var --name "ubuntu-2204-cloud-template$1" --memory 2048 --cores 2 --agent 1 --net0 virtio,bridge=vmbr0 --ostype l26 
    qm set $vmid_var --scsihw virtio-scsi-pci
    if [ "$vmid_var" == "$vmid_0" ]
        then
            qm set $vmid_var --cicustom "user=resources:snippets/ubuntu-user-config.yml"
            qm importdisk $vmid_var $ubuntu_qcow2 VM_Storage
...
until [ $( qm guest exec $vmid_0 sudo systemctl status cloud-final | grep -c "Finished" ) -eq 1 ]
    do 
...
qm guest exec $vmid_0 -- /bin/bash -c 'sudo cat /dev/null > /etc/machine-id && sudo cat /dev/null > /var/lib/dbus/machine-id && sudo cloud-init clean'
qm guest exec $vmid_0 -- /bin/bash -c 'sudo shutdown -h now'
qm wait $vmid_0
